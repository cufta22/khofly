    ...
    <IAWrapper>
      <Paper p='xs' w='100%' h={100} pos='relative'>
        ...
        <Flex h='100%' justify='flex-end' align='flex-end' direction='column'>
          <Text
            c={result.length === 0 ? '' : 'dimmed'}
            // size={!result.length ? "xl" : "md"}
            fz={result.length === 0 ? 30 : 16}
            fw={result.length === 0 ? 600 : 400}
          >
            {equation}
          </Text>

          {/* Slide in result */}
          <Transition transition='pop' duration={300} mounted={result.length > 0}>
            {(transitionStyles) => (
            ...
            if (gridBtn.variant === 'scientific' && variant === 'basic') return null;
              ...
              return (
                <Grid.Col key={gridBtn.value} span={'auto'}>
                  ...
                  >
                    {WIP_BUTTONS.includes(gridBtn.value) ? '' : gridBtn.label}
                  </Button>
                </Grid.Col>
              );
          })}
